...
(3) These articles are fed to another document encoder, and the resultant article embeddings are passed to an article aggregator to attentively select supportive articles, and produce the aggregated article embedding $\mathbf{d}_a$. Meanwhile, three context vectors, i.e., $\mathbf{u}_{aw}$, $\mathbf{u}_{as}$ and $\mathbf{u}_{ad}$, are dynamically generated from $\mathbf{d}_f$ for the article document encoder and the article aggregator. 
...
A challenge in building a sequence encoder is how to take the correlation of each elements into consideration. A promising solution is the Bi-GRU model proposed by \cite{bahdanau2015neural}, which encodes the context of each element by using a gating mechanism to track the state of sequence.
...
\orange{By using this sequence encoder for fact embedding, the fact-side attention module stack actually contains two components, i.e., the word-level and sentence-level attention modules using $\mathbf{u}_{fw}$ and $\mathbf{u}_{fs}$ as global context vectors respectively.}
