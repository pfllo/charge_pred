...
\cite{lin2012exploiting} propose to make deeper understanding of a case by identifying charge-specific factors \red{that are manually designed for two charges}. This method also suffers from the scalability issue due to the human efforts required to design and annotate these factors for each pair of charges. Our method, however, employs GRU and a \red{stack attention mechanism} to make comprehensive understanding of a case without relying on explicit human annotations. 
...
rerank the results using the similarity between the words in facts and articles, and \red{the correlations among law articles as indicators????}.
...
More importantly, \red{we optimize the article extraction task within our charge prediction framework,  which not only provides another view to understand the fact, but also serves as legal basis to support the final decision. }
...
\cite{yang2016hierarchical} further use global context vectors \red{as attention} to distinguish informative words or sentences from non-informative ones during embedding, which we share similar spirit with. 
