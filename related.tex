...
rerank the results using the similarity between the words in facts and articles, and \orange{the co-occurence tendency} among law articles as indicators.
% use some reranking methods to get the final relevant article list. 
We also utilize SVM to extract top $k$ candidate articles, but further adopt a bi-directional GRU architecture and an article-level attention mechanism to better understand the texts as well as exploring the correlation among articles. 
