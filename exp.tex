...
Let us consider an ideal situation where we can access both fact descriptions and gold standard law articles \orange{during testing}, which could be considered as an upper bound scenario.
...
by \orange{over} 8\%, and no surprisingly, \texttt{NN\_article} by more than 10\%.
%
...
There are usually 
\orange{clear} %significant 
differences between the expressions used by legal practitioners and people without legal background, 
thus it is important to see how our model will perform on fact descriptions \orange{written} by non-legal professionals.
%
...
It is not surprising that our NN models also suffer from the \orange{news genre}, but with about 10\%$\sim$15\% less absolute drop in F1, and our  \texttt{NN\_supv\_article} can still achieve 79.12\% in F1, \orange{thanks to our NN architectures}.
