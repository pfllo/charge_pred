...
By contrast, our \red{plain} neural network model (\texttt{NN}) outperforms \texttt{SVM} 
...
\red{even there is less overlap in the words used between cases with the same charge, 
or there are limited training data for infrequent charges. This may also explain that NN based models
have more balanced improvement across different charges, leading to more prominent improvements over 
...
When we use both facts and the extracted relevant law articles (that are admittedly noisy),  
...
Furthermore, if we use the gold standard articles during training to \red{supervisedly} train the 
...
\red{\texttt{NN\_article\_only} utilizes fact embeddings to attentively aggregate relevant articles, but only use 
...
variants (though still better than \texttt{SVM}). \red{This  indicates that  it is necessary to consider both fact description and 
relevant law articles for charge prediction, and,
although the judgments are made based on the statutory laws in the civil law systems, 
 the \textbf{logical thinking or legal reasoning} of the court behind the formal decisions, 
to some extent, may be implicitly captured through the fact-charges paris.
}


\red{Now  a question comes out: \textit{how much improvement can we have by expoiting the relevant law articles within the civil law system?} }
...
\emph{intentional homicide} (when the victim is dead, and the difference is %it differs from \emph{intentional injury} in
